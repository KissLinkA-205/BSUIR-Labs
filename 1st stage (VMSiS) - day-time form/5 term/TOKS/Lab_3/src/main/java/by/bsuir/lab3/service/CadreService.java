...
public class CadreService {
    ...
    public String[] splitDataToCadres(String data) {
        int cadreSize = NUMBER_OF_CONTROL_BITS + CADRE_LENGTH + 1;
        int cadreNumber = data.length() / cadreSize;
        String[] result = new String[cadreNumber];
        for (int i = 0; i < cadreNumber; i++) {
            result[i] = data.substring(cadreSize * i, cadreSize * i + cadreSize);
        }
        return result;
    }
